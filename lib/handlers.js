          ...
          _data
            ...
            .then(userData => {
              _data
                .delete("users", phone)
                .then(() => {
                  // Delete each one of the checks associated with the user
                  const userChecks =
                          ...
                          if (checksDeleted === checksToDelete) {
                            if (!deletionErrors) {
                              ...
                              reject({
                                statusCode: 500,
                                Error:
                                  "Error encountered while attempting to delete all of the users checks. All checks may not have been deleted from the system successsfully"
                              });
                            }
                          }
                        });
    ...
    if (id) {
      ...
      _data
        ...
        .then(checkData => {
          ...
          handlers._tokens
            .verifyToken(token, checkData.userPhone)
            .then(() => resolve({ statusCode: 200, payload: checkData }))
            .catch(() => reject({ statusCode: 403 }));
        })
        .catch(err => {
          console.log(err);
          reject({ statusCode: 404, Error: "Check not found" });
        });
    } else reject({ statusCode: 400, Error: "Missing required fields" });
  });
...
handlers._checks.put = data =>
  new Promise((resolve, reject) => {
    ...
    // Check to make sure id is validç
    if (id) {
      ...
      if (protocol || url || method || successCodes || timeoutSeconds) {
        ...
        _data
          ...
          .catch(err => {
            console.log(err);
            reject({ statusCode: 400, Error: "Check ID did not exist" });
          });
      } else reject({ statusCode: 400, Error: "Missing fields to update" });
    } else reject({ statusCode: 400, Error: "Missing required field" });
  });
